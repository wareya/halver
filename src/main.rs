...
");
    ...
    let measure_tile = |tx, ty, size, img : &image::RgbaImage|
    {
        ...
        for iy in 0..size
        {
            for ix in 0..size
            {
                let px = luma(&img.get_pixel(ix, iy).to_rgba());
                rms += px * px;
            }
        }
        rms = rms.powf(0.5);
    ...
    let dist2 = |x1 : f32, y1 : f32, x2 : f32, y2 : f32| OrderedFloat::from((x1-x2).powi(2) + (y1-y2).powi(2));
    ...
    for iy in 0..in_dim.1/in_size
    {
        for ix in 0..in_dim.0/in_size
        {
            ...
            tiles.sort_by_key(|tile|
            {
                ...
                dist2(m.0, m.1, m2.0, m2.1)
            });
            write_tile(ix, iy, &tiles[3]);
        }
    }
